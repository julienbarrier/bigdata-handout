...
this is what we did in the first lecture: compute \alpha and \beta that gave us
the best result.
...
thus y = e^{\omega^T x} (1-y)
therefore y(1+e^{\omega^T x}) = e^{\omega^T x}
...
u = \sum_{j=1}^p \omega_j x_j = \omega^T x is called the activation.
...
with (\ldots) = \frac{ k_i (1-y_i) - (1-k_i)y_i}{y_i (1-y_i)} = \frac{k_i - y_i}{y_i (1-y_i)} 

and \frac{\partial y_i}{\partial \omega_j} = \phi'(u_i) \frac{\partial u_i}{\partial \omega_j} = \phi'(u_i) X_{ij}
...
problem formally attitred: \min_\omega \sum_i \max(0,1-k_i \omega^T x_i ) + \lambda ||\omega||^2.
