...
I(\theta)_{ij} = - \frac{\part^2 \mathcal{L}}{\part \theta_j \part\theta_k}
...
\begin{marginfigure}
    \TODO
    figure parabolle inversée, maximum : courbature -\part\mathcal L/\part \theta^2, absisse max : \theta 0. ordonnée : L.
\end{marginfigure}
...
commentary:
I(\theta) = -\frac{\part^2 \mathcal{L}}{\part \theta_i \part \theta_j}
...
considering all the possibilities of my data.If we are artificially generating
data 
...
scheme fig 2.
If we look at the gradient, and start from a point (random). we look at the
grandient, and move in the direction of it.
we usually take a very small displacement. start iteratively.

\theta_{t+1} = \theta_{t} - \eta \left( \frac{\part L}{\part \theta}\right)_{\theta_{t}}
...
This is really a local method. If I start from one point, I can end somewhere
...
convergenc.
we get an interative algorithm. beta 1, then beta2, \ldots
